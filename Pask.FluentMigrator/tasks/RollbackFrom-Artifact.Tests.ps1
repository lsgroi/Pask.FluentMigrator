...
Describe "RollbackFrom-Artifact" {
    ...
    Context "Extract and default rollback from artifact" {
        BeforeAll {
            # Arrange
            $Connection = New-Object -TypeName System.Data.SQLite.SQLiteConnection
            $Uri = New-Object -TypeName System.Uri (Join-Path $TestDrive "database.db")
            ...
            Invoke-Pask $TestSolutionFullPath -Task Extract-Artifact, RollbackFrom-Artifact -ProjectName ClassLibrary.DatabaseMigrations -MigrationDatabase "SQLite" -MigrationConnectionString $Connection.ConnectionString
        }
